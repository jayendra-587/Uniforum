...
$get_topics_res = mysqli_query($conn, $get_topics_sql) or die(mysqli_error($conn));

$display_read = "<a href = 'readThreadList.php><button id = 'read' class = 'styledButton'>Read Saved Topics</button></a>";
$display_save = "";
...
END_OF_TEXT;
    ...
    while ($topic_info = mysqli_fetch_array($get_topics_res)) {
        ...
        $get_num_posts_res = mysqli_query($conn, $get_num_posts_sql) or die(mysqli_error($conn));
...
END_OF_TEXT;
            ...
            $display_block .= <<<END_OF_TEXT
			<tr>
			<td><a href="showThread.php?topic_id=$topic_id"><strong>$topic_title</strong></a><br/>
			Created on <label> $topic_create_time </label> <br> <i id = "userLogo" class = "fa fa-user"></i> <u> $topic_owner </u></td>
			<td class="num_posts_col">$num_posts</td><td class="num_posts_col" style = 'min-width: 60px;'><a id = "like" href = "like.php?topic_id=$topic_id"><i class = "fa fa-heart-o"></i></a> $topic_likes</td>
END_OF_TEXT;
            if($topic_owner == stripslashes($_SESSION["user"])){
                $display_block .= '<td class="num_posts_col">  <a id = "edit" href = "editThread.php?topic_id='.$topic_id.'"><i class = "fa fa-edit"></i></a> <a id = "delete" href = "deleteThread.php?topic_id='.$topic_id.'"><i class = "fa fa-trash"></i></a></td>';
            }
            else{
                $display_block .= '<td class = "num_posts_col">Not <br> Available</td>';
            }
